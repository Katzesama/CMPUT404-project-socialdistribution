...
class FriendRequestHandler(APIView):
    ...
    def post(self, request):
        ...
        if not is_friend:
            if FriendRequest.objects.filter(url=receiver_obj.url, friend_with=sender_obj, friend_status="proceeding").exists():

                    friend_request = FriendRequest.objects.filter(url = receiver_obj.url, friend_with=sender_obj)[0]
