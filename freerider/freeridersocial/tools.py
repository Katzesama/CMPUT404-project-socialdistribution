...
def create_local_author(data):
    ...
    if not Author.objects.filter(id = UUID(data['id'].split('author/')[1])).exists():
        author = Author.objects.create(id=UUID(data['id'].split('author/')[1]), displayName=data["displayName"], url = data['url'], host=host)
        author.save()
    else:
...
def reply_remote_friendrequest(data, node):
    ...
    response = requests.post(url, headers=header, data=json.dumps(data), auth=HTTPBasicAuth(node.username,node.password))
    print('aaaaa')
...
#def get_requestor_info_with_url(request, url):
    ...
    #     current_author_host = request.scheme + '://127.0.0.1:7000'
    #
    # request_url = current_author_host + '/author/' + id + '/api/'
    # #print(request_url)
    # node = ServerNode.objects.get(pk=current_author_host)
